...
func (b *Bot) handleTrainingAddSet(c tele.Context) error {
	...
	prompt := "Отправь обычный подход: 10Р 60КГ или 10Р -"
	if exercise := session.CurrentExercise(); exercise != nil {
		if weight, ok := exercise.LastWorkingWeightKG(); ok {
			lastWeight := "собственный вес"
			if weight != nil {
				lastWeight = strconv.FormatFloat(*weight, 'f', -1, 64) + " кг"
			}
			prompt = "Отправь повторы числом, чтобы повторить " + lastWeight + ", или весь подход: 10 либо 10Р 60КГ"
		}
	}
	return b.showActiveTraining(ctx, c, session, prompt)
}
...
func (b *Bot) handleTrainingRIR(c tele.Context) error {
	...
	defer cancel()
	ownerID := c.Sender().ID
	session, err := b.deps.Training.CompletePendingSet(ctx, ownerID, &value, time.Now())
	if err != nil {
		return b.showTrainingFailure(ctx, c, ownerID, err)
	}
...
func (b *Bot) handleTrainingCancelSet(c tele.Context) error {
	b.respond(c)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	ownerID := c.Sender().ID
	...
	return b.showActiveTraining(ctx, c, session, "Обычный подход не сохранён")
}
...
func (b *Bot) handleText(c tele.Context) error {
	...
	case training.InputSet:
		raw := c.Text()
		b.deleteIncoming(c)
		session, err := b.deps.Training.PrepareWorkingSetInput(ctx, ownerID, raw)
		if err != nil {
			if active, activeErr := b.deps.Training.Active(ctx, ownerID); activeErr == nil {
				return b.showActiveTraining(ctx, c, active, "Не понял обычный подход: "+err.Error())
			}
			return b.showTrainingFailure(ctx, c, ownerID, err)
		}
		return b.showActiveTraining(ctx, c, session, "Какой был RIR?")
	...
	case training.InputRIR:
		b.deleteIncoming(c)
		if active, activeErr := b.deps.Training.Active(ctx, ownerID); activeErr == nil {
			return b.showActiveTraining(ctx, c, active, "Выбери RIR кнопкой")
		}
...
func (b *Bot) handleTrainingHistory(c tele.Context) error {
	...
	if len(page.Items) == 0 {
		...
		for _, session := range page.Items {
			...
			setsLabel := fmt.Sprintf("обычных: %d", working)
			if warmup > 0 {
				setsLabel = fmt.Sprintf("обычных: %d + разминочных: %d", working, warmup)
			}
			fmt.Fprintf(&text, "\n• %s · %s · %s · %s",
...
func (b *Bot) showActiveTraining(ctx context.Context, c tele.Context, session training.Session, prompt string) error {
	...
	if state.Mode == training.InputRIR && state.PendingSet != nil {
		rows = append(rows,
			...
			markup.Row(markup.Data("Отмена", trainingCallbackCancelSet, exerciseID)),
		)
	} else {
		rows = append(rows,
			markup.Row(markup.Data("➕ Разминочный подход", trainingCallbackAddWarmup, exerciseID)),
			markup.Row(markup.Data("➕ Обычный подход", trainingCallbackAddSet, exerciseID)),
			markup.Row(
...
func (b *Bot) showImportPrompt(ctx context.Context, c tele.Context, ownerID int64, notice string) error {
	...
	text := "<b>📥 Импорт программы</b>\n\nОтправь YAML файлом, обычным сообщением или код-блоком. Также поддерживаются старые .txt и .csv до 1 МБ.\n\n" +
		"YAML version 1 может содержать несколько тренировочных дней и настройки плана. Во время тренировки разминочные и обычные подходы добавляются вручную."
	if notice != "" {
		text += "\n\n<b>" + html.EscapeString(notice) + "</b>"
	}
	markup := &tele.ReplyMarkup{}
...
func (b *Bot) trainingPrompt(ctx context.Context, ownerID int64) string {
	...
	switch state.Mode {
	case training.InputWarmup:
		return "Отправь разминочный подход: 10Р 20КГ или 10Р -"
	case training.InputSet:
		return "Отправь обычный подход: 10Р 60КГ или 10Р -"
	case training.InputRIR:
		return "Выбери RIR кнопкой"
	case training.InputNote:
